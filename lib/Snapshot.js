...
//TODO pass options (additional discards, timeout, ...)
/**
 ...
 * @constructor
 */
function Snapshot(uri, snapshotFolder, options) {
    this.uri = uri;
    ...
    this.saveHtml = true;
    if (options) {
        //this.discardLink += options.discardLink ? ('|(' + options.discardLink + ')') : '';
        this.siteTimeout = options.siteTimeout ? options.siteTimeout : siteTimeout;
        this.saveHtml = options.saveHtml !== undefined ? options.saveHtml : this.saveHtml;
    ...
    this.recursiveQueue(this.uri, function () {
        ...
        for (var link in this.linkQueue) {
            if (this.linkQueue.hasOwnProperty(link)) {
                ...
                /* remove all links which got discarded */
                else if (this.linkQueue[link].discard) {
                    delete this.linkQueue[link];
                }
            }
        }
        ...
        for (var link in this.linkQueue) {
            if (this.linkQueue.hasOwnProperty(link)) {
                ...
                if(this.changefreq) {
                    sitemap += '    <changefreq>' + this.changefreq + '</changefreq>\n';
                }
        ...
        mkpath(this.snapshotFolder, function (err) {
            if (err) {
                debug(err);
            }
            else {
...
Snapshot.prototype.recursiveQueue = function (uri, cb) {
    /* generateSinglePage or getPage calls handleQueue */
    var handleQueue = function (err, result) {
        ...
        for (var link in this.linkQueue) {
            if (this.linkQueue.hasOwnProperty(link)) {
                if (!this.linkQueue[link].visit) {
                    if (!link.match(this.discardLink)) {
    ...
    if (this.saveHtml) {
        ...
        this.generateSinglePage(uri, handleQueue);
    }
    else {
        /* if we don't want to generate the page just get the result */
        this.getSite(uri, handleQueue);
    }
};
...
Snapshot.prototype.generateSinglePage = function (uri, cb) {
    this.getSite(uri, function (err, result) {
        if (err) {
            debug(err);
            cb(err);
            return;
 ...
 * @param cb (err,result) result = {location, links, content, origin}
 */
Snapshot.prototype.getSite = function (uri, cb) {
    var siteTimeout = this.siteTimeout;
    phantom.create({parameters: {'ignore-ssl-errors': 'true'}}, function (ph) {
        ph.createPage(function (page) {
            ...
            page.open(uri, function (status) {
                ...
                /* jquery has to be reachable from the snapshot website -> googleapis */
                page.includeJs('http://ajax.googleapis.com/ajax/libs/jquery/2.1.1/jquery.min.js', function () {
                    ...
                    setTimeout(function () {
                        ...
                        }, function (result) { /* result from evaluate */
                            ph.exit();
